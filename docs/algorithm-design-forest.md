...
- **检索内核特化**：当前实现原生特化于**全库开放检索与阈值检索（Open / Threshold Search）**，通过连续内存的 SIMD 批量根求值（`batch_root_bounds`）、JIT 叶上界内核、动态门槛 $\theta$ 贪心预植入与全局 Best-First 优先队列跨树动态推进，以严格零漏检（Zero False Dismissals）实现中位数 48.63 ~ 51.22 ms（均值 85.61 ~ 97.91 ms，QPS 10.2 ~ 11.7）的极致高速检索。
...
1. **模态一：前体二分靶向检索（Targeted Precursor Mode）**：当指定 `QueryConfig(precursor_window=PrecursorWindow(mz=..., tolerance_da=...))`（亦支持 `min_mz=..., max_mz=...` 边界传参）时，利用 `ForestTrees.precursor_min/max` 升序数组在 Stage 0 执行二分定位（$O(\log M)$），瞬间排除 $>96\%$ 的无关小树，候选树仅需展开 1~2 棵；
...
2. **阶段 2：Top-K 动态门槛 $\theta$ 贪心预植入与探测开销追踪（Dynamic $\theta$ Pre-Seeding & Probe Tracking）**：
   - **设计机理**：在 Top-K 开放检索模式初期，堆未满时剪枝门槛 $\theta = -\infty$。系统贪心探测顶层前体候选树（默认前 3 棵），按叶包络上界降序直接精评叶微块谱（至多 $\text{probe\_trees} \times \text{tree\_capacity}$ 条，如 $3 \times 64 = 192$ 条以内），迅速发现高质量真实匹配；
   - **阈值快速抬升**：在第 0 轮迭代即可将动态门槛 $\theta$ 从 $-\infty$ 快速抬升至真实候选下界（通常为 0.60 ~ 0.85+），换取后续全局遍历中 **>99.9% 的极致剪枝率**，使得堆内 $90\%+$ 的低分小树在尚未展开时即被批量整树剪除；
   - **开销完全透明**：探测阶段精评的谱数如实计入统计指标 `SearchStats.n_scored`，并同时记录在 `SearchStats.probe_scored` 中进行独立透明追踪，确保用户对精评开销与来源清晰可审计，消除“探测精评是否免费”的疑虑；
   - **计算零冗余**：被 probe 探测的小树在探测阶段已完成其全部叶节点的评估与剪枝，后续无需也不会重新放入优先队列 `pqueue`，全生命周期绝不产生重复计算。
...
| **端到端中位数时延** | 严重长尾超时 | ~25,000 ms (25s) | **48.63 ~ 51.22 ms (单线程中位数，均值 85.61 ~ 97.91 ms，QPS 10.2 ~ 11.7；16 线程并发批处理中位数 381.24 ~ 484.78 ms，QPS 8.8 ~ 10.6)** | **< 2.0 ms (QPS > 500)** |
